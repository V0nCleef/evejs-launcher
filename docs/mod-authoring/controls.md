...
Add this field to your list of form fields array. It uses the `prefs` file declared on the [Configure page](configure.md). Players see **Low** or **High**; the file stores `"low"` or `"high"`.
...
Give every field a valid default and useful limits. Invalid existing values produce an error rather than silently becoming defaults. See [3. Add a Configure button](configure.md).
