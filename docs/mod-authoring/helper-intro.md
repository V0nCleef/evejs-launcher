...
Get `profile-options` from [15. Example files](example-files.md). This example turns a saved preference into a setting used when a profile starts. The helper receives the chosen values and sends back its proposed changes. The launcher checks those changes before saving them.
