...
Get `source-overlay-demo` from [15. Example files](15-example-files.md). Try it only in a spare test installation. Its form lets you choose which example section to change and the replacement text. It demonstrates changing one owned part of a server file and restoring it later.

![Demo screenshot — open to enlarge](images/source-options.png)

[Open full-size image](images/source-options.png)

An existing source patch can use on/off value in a settings file instead. The mod must check its enabled flag before creating entities, timers or other effects. [1. Make your first package](01-first-mod.md).
