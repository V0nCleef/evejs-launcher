...
from .local_mod_packages import LocalModPackages, LocalModRecord
...
def load_mod_inventory(root: str) -> ModInventory:
    ...
    return ModInventory(root, tuple(mods), state, state_error, management, errors,
                        local, frozenset(removable), quarantined, registry_error, pending, update_pending, evejs_version)
